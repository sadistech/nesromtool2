...
nrt_chr_listitem* nrt_chr_list_delete(nrt_chr_list* list, int index)
{
  // first get the item in question
  nrt_chr_listitem* item = nrt_chr_listitem_at(list, index);
  ...
  // get the previous item, set previous->next to item->next
  list->count--;
  nrt_chr_listitem* prev = nrt_chr_listitem_at(list, index - 1);
  prev->next = item->next;
